...
class ParastorConnector(RemoteConnector):
    ...
    def remove(
        ...
        size = aiofiles.os.path.getsize(file_path)
        self.usage -= size
        aiofiles.os.remove(file_path)
        
        logger.debug(f"parastor rm kv path: {file_path}, len: {size}")
    async def get(self, key: CacheEngineKey) -> Optional[MemoryObj]:
        ...
        try:
            async with aiofiles.open(file_path, "rb") as f:
                ...
                logger.debug(f"parastor get kv path: {file_path}, len: {num_read}")
    ...
    async def put(self, key: CacheEngineKey, memory_obj: MemoryObj):
        ...
        try:
            ...
            size = len(buffer) + METADATA_BYTES_LEN
            self.usage += size
            
            logger.debug(f"parastor put kv path: {final_path}, len: {size}")
